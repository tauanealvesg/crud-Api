...
export const getPet = (_, res) => {
  ...
  export const updatePet = (req, res) => {
    const q = 
      "UPDATE crud.pet SET `cpf_user` = ?, `nome_pet` = ?, `raca` = ?, `nascimento_pet` = ? WHERE `id_pet` = ?";
    ...
    db.query(q, [...values, req.params.id_pet], (err) => {
      if (err) return res.json(err);
  ...
  export const deletePet = (req, res) => {
    const q = 
      "DELETE FROM crud.pet WHERE `id_pet` = ?";
  
    db.query(q, [req.params.id_pet], (err) => {
      if (err) return res.json(err);
